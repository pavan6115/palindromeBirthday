...
function clickHandler(){
    ...
    if(bDayStr !== '') {
        ...
        if (isPalindrome){
            outputMsg.innerText = "Yuhuu 🥳, it is a Palindrome"
        }
        else {
            let [count, nextDate] = nextPalindromeDate(dateObj)
            outputMsg.innerText = `Ohh no 😓, it isn't a Palindrome. The next palindrome date is ${nextDate.day}-${nextDate.month}-${nextDate.year}. You missed it by ${count} days 😔`
        }
    }
}
